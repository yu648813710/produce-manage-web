...
<template>
  <div>
    ...
    <a-modal
      ...
      cancelText="取消"
    >
      <div>是否发布当前方案到{{solutionPlan.solutionScope === 'market' ? '公开市场' : '公司私有'}}？</div>
    </a-modal>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getProjectDetail() {
      projectDetail(this.detail.solutionId).then(res => {
        ...
        for (
          let i = 0;
          i < detailData.solutionPlanCycleMaterialList.length;
          i++
        ) {
          detailData.solutionPlanCycleMaterialList[i].actionCycle =
            detailData.solutionPlanCycleMaterialList[i].taskStartDay +
            '天-' +
            detailData.solutionPlanCycleMaterialList[i].taskEndDay +
            '天'
        }
